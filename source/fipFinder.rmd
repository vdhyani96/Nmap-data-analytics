...
Our network was configured with subnet masking in such a way that we had only 254 IP addresses available on our network to connect to the internet. And the number of students staying in the hostel was also around 250, with almost everyone having at least one device that could be connected to the internet (laptop, smartphones, etc.). This, however, is not the problem itself, because not everyone was connected to the internet at the same time from all their devices, but it definitely contributed to the problem. 

We also had static IP configured on our network, meaning that anyone could manually change their IP addresses at any point of time. It was not automatically assigned by a DHCP on our Wi-Fi routers. This was the primary reason of the problem. Almost no one used to stay consistently on a single IP address and kept flitting from one IP to another until they could find a *"free"* IP address. This led to problems for others who had saved that IP address on their devices and no longer could connect using it because someone else had *"occupied"* it! This eventually led everyone to be on the continuous run and be a nomad on the network. This IP conflict problem was definitely a frustrating experience for many users who, even after manually checking multiple IP addresses, couldn't find a single working IP! 

## Exploring for a Solution

Talking to the authorities was definitely one of the ways to sort this out, but it'd take a long time for them to understand and take any effective action, so we just let them know of the situation, but never expected any outcome from that.

On the other hand, I was searching for a solution that would give me a list of free IP addresses on that instant of time, so that I wouldn't have to manually search the entire spectrum of the IP list. I tried searching for some networking utilities for this task. Eventually, I came across the [NMAP](https://nmap.org/) software, or Network Mapper, a network port scanner that could give me information about the network. When connected on the network, I could perform an ARP ping scan to get a list of IP addresses that are in use currently, and a list of MAC addresses (or devices) which are currently using those IP addresses. 
This single command performed the task: `nmap -sP -PR 10.18.1.*`

## The Data Science Angle

The data that I was getting from NMAP was very valuable. It was the instantaneous detail of the network and IP statuses. I recognized that if had enough data, I could figure out a way to feed it to a Machine Learning algorithm to make predictions about which IPs will most likely be free and which ones won't be. Moreover, I would also be able to find any interesting patterns in the usage of the WLAN network by different users on different days. 

# Data Gathering

The thought of putting my newly acquired Machine Learning knowledge into work really got me excited. I began collecting the NMAP data daily and storing them in separate text files. I used the `clip` CMD command for that (clip for clipboard most probably). In the beginning, I gathered the data at around 9pm daily. But that was not the peak time and many users weren't on the network in that hour. So, with the expectation of capturing the usage pattern on a larger extent, I shifted the time to 11pm and continued gathering the data. For more than two months, or 66 days to be exact, I collected the network data and then stopped as I thought it would be good enough to capture the general pattern in the network. 
...
From the text files, I first select 4 potentially useful attributes:
1. Date
2. Time
3. IP Addresses
4. Latency
...
# reading the file
dfTest <- readLines("0331.txt") %>% as.data.frame()
...
# For latency, first finding median latency per statuses and then imputing for the selective statuses in the misData dataframe by taking inner join
...
Final counts of datasets in the two categories become 24 for 9pm and 42 for 11pm, with the total being 66 days. At this point, I get two combined datasets that are ready for analysis. The data looks something like this:
```{r, echo = FALSE, warning = FALSE, message = FALSE}
NineDf <- read.csv("C:/Users/admin/Desktop/R/My NMAP Project/Datasets/ninePmDataset.csv")
```
...
After defining the function, I ca use it for the 3 datasets and then print the results on a barplot. 
...
We can see the answer to our question in the above plots. Moreover, we also get some other important insights. As can be clearly seen from above, all the days in the second half of the week (Thu, Fri, Sat and Sun) have more number of free IPs than the first half (Mon, Tue, Wed). It is fairly consistent among all the plots, except for some deviation in the 9pm data where Sunday count seems to be a bit on the lower side. Also, we see overall less number of free IPs in case of 11pm data because this is kind of peak time as compared to 9pm. 
...
The answer to this question can give us an insight into who the people, or their machines, are that are the most active on the network, regardless of which IP addresses they are using. It would also be interesting to explore which specific IPs they use to stay the most active on the network. I won't consider the first (10.18.1.1) and the last IP addresses (10.18.1.254) in this analysis because they are reserved for special purposes and will be found active all the time with the same `Status`. Also, there is one more IP address on the network that is always found active with the same status: 10.18.1.251. I am not sure about the exact reason but it is also probably not a normal user. So, I will ignore it too. Lastly, I will always be found active on the network as well, because, at the time I am collecting the data, I myself have to be active on one of the free IPs.

Like the last time, I will only use the combined dataset. To find the most active MAC addresses, I first find the count of the MAC addresses across all the days and then sort them according to their counts. 
...
The system I have developed works well in this current scope. Although we can't directly verify the predictions of the model for the future examples, as the usage pattern may change and the hostel batch will change and therefore, a different set of people will start using the network, we may find that the predictions are correlated to what we found earlier in the data analysis about the most and least frequent IP addresses on different days. Moreover, the machine learning system has the capacity to accommodate well for the future examples too, provided more new examples are fed to it after the batch changes or some other major change occurs. 
...
// before doing analysis on here, can show the 'head(6)' of the final dataset.

// Try to make some good outlines for this report.
// keep loading the libraries on the top
// can mention a note that I just wrote down today in notebook (19-04-18)
